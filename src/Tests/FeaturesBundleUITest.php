...
class FeaturesBundleUITest extends WebTestBase {
  ...
  public function testNewAssignmentConfigure() {
    ...
    // Is the method really removed?
    $settings = $this->defaultBundle()->getAssignmentSettings('exclude');
    $this->assertIdentical($settings, [], 'No exclude assignment');
    ...
    // Check new settings.
    $settings = $this->defaultBundle()->getAssignmentSettings('exclude');
    $this->assertTrue($settings['enabled'], 'Exclude enabled');
    $this->assertTrue(isset($settings['weight']), 'Exclude has weight');
    $other_keys = array_diff(array_keys($settings), ['enabled', 'weight']);
    $this->assertIdentical($other_keys, [], 'Exclude has no other settings');
